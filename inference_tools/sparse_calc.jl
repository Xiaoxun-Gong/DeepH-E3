...
if isfile(joinpath(parsed_args["input_dir"], "sparse_matrix.jld"))
    ...
    if true
        ...
        for key in keys(fid_H)
            hamiltonian_pred = permutedims(read(fid_H[key])) # npz似乎为julia专门做了个转置而h5没有做
            overlap = permutedims(read(fid_S[key])) # npz似乎为julia专门做了个转置而h5没有做
